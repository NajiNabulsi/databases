...
for (let i = 0; i < authors.length; i++) {
    ...
    connction.query(`insert into Authors values (${authors[i].author_no},'${authors[i].author_name}', '${authors[i].university}', '${authors[i].date_of_birth}', ${authors[i].h_index},'${authors[i].gender}' ,${authors[i].frinds})`, err => {
        if (err) throw err;
    });

};

connction.query('CREATE TABLE if not exists Research_Papers(paper_id int Primary Key not null, paper_title varchar(50), conference varchar(50),       publish_date date, author_no int, FOREIGN Key(author_no) REFERENCES Authors(author_no))', err => {
    if (err) throw err;
});

for (let i = 0; i < researches.length; i++) {

    connction.query(`insert into Research_Papers values (${researches[i].paper_id}, '${researches[i].paper_title}','${researches[i].conference}', '${researches[i].publish_date}', ${researches[i].author_no})`, err => {
        if (err) throw err;
    });
